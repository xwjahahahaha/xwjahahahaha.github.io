...
![libs_to_cncf_arch](http://xwjpics.gumptlu.work/qinniu_uPic/libs_to_cncf_arch.png)
...
- libsinsp：对于从 libscap 上报的底层事件进行信息获取、补充并发送至 Rule Engine进行过滤；
...
在单机 Falco 部署以后，我们更多的场景是**将所部署 Falco 主机的日志或者告警进行集中式管理**，`FalcoSideKick` 项目就是为此而生。
...
![evt_collect_display](http://xwjpics.gumptlu.work/qinniu_uPic/evt_collect_display.png)

FalcoSideKick UI 的界面如下：

![falcosidekick-ui](http://xwjpics.gumptlu.work/qinniu_uPic/falcosidekick-ui.png)
...
![image-20220712150626747](http://xwjpics.gumptlu.work/qinniu_uPic/image-20220712150626747.png)
...
```shell
# 在另外一个窗口执行 
$ sudo docker exec -ti fd8bf26a45be  /bin/bash -il
# falco 日志中打印以下内容：
08:38.611294913+0000: Notice A shell was spawned in a container with an attached terminal (user=root user_loginuid=-1 intelligent_shockley (id=fd8bf26a45be) shell=bash parent=runc cmdline=bash -il terminal=34817 container_id=fd8bf26a45be image=falcosecurity/falco)
```
...
falco 从 0.13 版本开始支持 k8s audit 日志，启动一个内部内嵌的服务端，用于接受 k8s audit 日志输出，这需要在 k8s 中定义相关的规则将 audit 日志提交至该服务，然后经相关规则定义后，输出可能触发的告警信息。 架构图如下：
