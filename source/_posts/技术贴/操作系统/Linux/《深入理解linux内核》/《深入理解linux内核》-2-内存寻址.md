...
段寄存器的唯一目的就是**存放段选择符(16位)**，这些寄存器称为：`cs、ss、ds、es、fs、gs`
...
所有的GDT都存放在`cpu_gdt_table`数组中，而所有GDT的地址与大小（即描述其的信息，初始化`gdtr`寄存器的时候使用）都存放在`cpu_gdt_descr`数组中
...
* 出现的原因？
  ...
  cpu的时钟周期频率是几个GHz，但是动态RAM的读取和写入的时间要慢的多，所以为了加速这个过程，就在CPU（**每个**处理器）的内部实现了一个SRAM的高速缓存内存
