 ...
 * }
 */
// 路径有三种，从左右某一个孩子加上自己向上（2种）
// 或者不再向上递归，左->中->右（1种）
// 对于每一个节数点三元组，找到这三种的最大
func maxPathSum(root *TreeNode) int {
    res := math.MinInt
    var help func(root *TreeNode) int   // 返回当前节点的最大路径和
    help = func(root *TreeNode) int {
        ...
        // 需要注意的点：如果左右孩子的最大路径和<0，那么还不如不要
        l = max(l, 0)
        r = max(r, 0)

        lrMax := max(l, r)
        nowMax := max(lrMax+root.Val, l+r+root.Val)
        if nowMax > res {
            res = nowMax
        }

        // 注意返回的是左右其中一个孩子+自己的和，不能返回nowMax，因为如果采用第三种方式不会再向上递归
