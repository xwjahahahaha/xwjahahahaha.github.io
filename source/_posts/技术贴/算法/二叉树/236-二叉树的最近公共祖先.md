...
func lowestCommonAncestor(root, p, q *TreeNode) *TreeNode {
    ...
    leftAns, rightAns := lowestCommonAncestor(root.Left, p, q), lowestCommonAncestor(root.Right, p, q)
...
// 非递归版本
func lowestCommonAncestor(root, p, q *TreeNode) *TreeNode {
    numsP, numsQ := backRange(root, p), backRange(root, q)
    minN := min(len(numsP), len(numsQ))
    var last *TreeNode
    for i:=0; i<minN; i++ {
        if numsP[i] != numsQ[i] {
            return last
...
// 二叉树的非递归后序遍历
func backRange(root, end *TreeNode) []*TreeNode {
    stack := []*TreeNode{}
