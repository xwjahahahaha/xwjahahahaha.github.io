...
func Constructor(capacity int) LRUCache {
    return LRUCache{capacity, newList(), make(map[int]int), make(map[int]*Node)}
}

func (this *LRUCache) Get(key int) int {
    if _, has := this.dataMap[key]; has {
        // 更新频率
        this.list.moveToTail(this.nodeMap[key])
        return this.dataMap[key]
    }
    return -1
}
...
func (this *LRUCache) Put(key int, value int)  {
    if _, has := this.dataMap[key]; has {
        ...
        if len(this.dataMap) == this.c {
            // 满了的情况
            // 删除最近最久未使用的
            node := this.list.deleteHead()
            delete(this.dataMap, node.Val)
            delete(this.nodeMap, node.Val)
        }
        // 加入
        newNode := &Node{key, nil, nil}
        this.list.addTail(newNode)
        this.dataMap[key] = value
...
注意LRU使用go语言写的问题，对于链表节点不要使用`ListNode`命令，会报错`undefined deserializer`，改成其他即可

详细见：https://leetcode.cn/circle/discuss/g9rfzf/
