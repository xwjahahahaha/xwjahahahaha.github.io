...
// topK大问题，堆
func findKthLargest(nums []int, k int) int {
    newMinHeap := minHeap([]int{0})
    for _, num := range nums {
        if len(newMinHeap) == k+1 { 
            if num > newMinHeap[1] {    // 如果堆满了并且比堆顶元素大则替换, 如果小于则什么也不做，所以这里的两个if不能合并
                newMinHeap.shiftDown()
                newMinHeap.shiftUp(num)
            }
        }else {
            // 堆没满则直接加入
            newMinHeap.shiftUp(num)
...
```go
// 快速选择
// 快速排序每次都能确定一个数在排序后的位置, 据此快速找到第k个而不需要将排序完成
func findKthLargest(nums []int, k int) int {
    return quickChoose(nums, 0, len(nums)-1, k)
}

func quickChoose(nums []int, l, r, k int) int {
    if l <= r {                                 // 细节：这里即使相等也再判断一次，因为可能就是答案，与快排不同的地方
        // 随机化
        randInt := rand.Intn(r-l+1)
        nums[l], nums[randInt+l] = nums[randInt+l], nums[l]

        mid := getMid(nums, l, r)
        idxFromBack := len(nums)-mid            // 注意这里返回的mid是正序的，而k是逆序计算的
        if idxFromBack == k {                
            return nums[mid]
        }else if idxFromBack > k {
            return quickChoose(nums, mid+1, r, k)
        }else {
            return quickChoose(nums, l, mid-1, k)
        }
    }   
