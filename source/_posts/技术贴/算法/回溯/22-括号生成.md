...
func generateParenthesis(n int) []string {
    ans := []string{}
    var dfs func(index int, c byte, path string, cl, cr int)
    dfs = func(index int, c byte, path string, cl, cr int) {
        path += string(c)
        if c == '(' {
            cl ++
        }else {
...
func generateParenthesis(n int) []string {
    res := []string{}
    i, j := 0, 0            // 分别记录了()的数量
    var dfs func(c string, path string)
    dfs = func(c string, path string) {
        fmt.Println(c, path, i, j)
        // 加入当前字符 
        path += c
