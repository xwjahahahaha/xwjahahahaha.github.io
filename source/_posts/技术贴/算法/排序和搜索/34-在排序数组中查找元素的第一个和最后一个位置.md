...
```go
func searchRange(nums []int, target int) []int {
    p1, p2 := binarySearch(nums, target, true), binarySearch(nums, target, false)
    if p1 == p2 {
        // 如果p1==p2，就说明一定没有==target的值，因为如果有target p1<p2
        return []int{-1, -1}
    }
...
// lower = true 表示找到>=target的第一个数的位置
// lower = false 表示找到>target的第一个数的位置
