...
二分思路见：https://leetcode.cn/problems/median-of-two-sorted-arrays/solution/xiang-xi-tong-su-de-si-lu-fen-xi-duo-jie-fa-by-w-2/
...
func findMedianSortedArrays(nums1 []int, nums2 []int) float64 {
    m, n := len(nums1), len(nums2)
    var help func(nums1, nums2 []int, k int) int      // help函数：找到两个排序数组的第k小的数
    help = func(nums1, nums2 []int, k int) int {
        m, n := len(nums1), len(nums2)
        // 一旦有一方为空，那么就返回另一个数组的第k-1个数
        if m == 0 {
            return nums2[k-1]
        }
        if n == 0 {
        ...
        // 注意，可能某一个数组的长度<k/2，那么此时就取其最后一个元素比较
        idx1, idx2 := min(k, len(nums1))-1, min(k, len(nums2))-1
        // 比较两个位置的数谁更大
        if nums1[idx1] < nums2[idx2] {
            // 排除掉nums1的左边的以及idx1位置的数
            // 更新k(以实际的index为准)
            k = preK - (idx1+1)
            nums1 = nums1[idx1+1:]
        }else {
