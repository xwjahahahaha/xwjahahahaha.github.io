...
```go
// 二分查找目标值的位置
// lower = true 表示找到>=target的第一个位置
// lower = false 表示>target的第一个位置
func binarySearch(nums []int, target int, lower bool) int {
	...
	for l < r {
		...
		if nums[mid] > target || (lower && nums[mid] >= target) {
			r = mid
			res = mid				// 虽然可能等于，但是不一定是第一个，所以不断往前挪
		} else {
			l = mid + 1
		}
	}
...
相关题目：[34. 在排序数组中查找元素的第一个和最后一个位置](https://leetcode.cn/problems/find-first-and-last-position-of-element-in-sorted-array/)
