...
除了资源限制，Cgroups还提供了不同资源使用优先级控制，审计等功能
...
对cpu资源的设置可以从2个维度考察：cpu使用百分比和cpu核数目。前者使用cpu subsystem进行配置，后者使用cpuset subsystem进程配置。
...
那是因为 COPY 比 ADD 更直观易懂。 COPY 只是将本地文件拷入容器这么简 单，而 ADD 有一些其它特性功能(诸如，本地归档解压和支持远程网址访问等)，这些特性在指令本身体现并不明显。因此，有必要使用 ADD 指令的最好例子是**需要在本地自动解压归档文件到容器中的情况**，如 `ADD rootfs.tar.xz`
...
* `CMD`: 容器启动时运行的命令
  ...
  > <font color='#39b54a'>RUN是构件容器时就运行的命令以及提交运行结果，CMD是容器启动时执行的命令，在构件时并不运行，构件时紧紧指定了这个命令到底是个什么样子</font>
