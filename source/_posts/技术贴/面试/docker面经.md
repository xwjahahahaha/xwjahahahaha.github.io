...
* 虚拟机本质上是一种模拟系统，模拟硬件的输入输出。每个虚拟机都需要模拟一个客户机内核，每个虚拟机使用一个单独的内核，开机前需要检查，开机较慢，占用内存大
* 从虚拟化的层次来说，虚拟机是硬件抽象层上的虚拟化
...
![image-20220325095253972](http://xwjpics.gumptlu.work/qinniu_uPic/image-20220325095253972.png)
...
* 完全公平的调度策略`CFS`
  ...
  * 按限额/使用上限（cpu使用周期的**绝对**限制）

    * `cpu.cfs_period_us`：用于设定周期时间（CFS的默认周期时间是100ms），必须与`cfs_quota_us`配合使用

    * `cpu.cfs_quota_us` ：用于设定周期内<font color='#e54d42'>最多</font>可使用的时间。这里的配置指任务对**单个cpu**的使用上限，`cfs_quota_us`是`cfs_period_us`的两倍，就表示在两个核上完全使用。数值范围为1000-1000,000（单位都是微秒）
    ...
    * `cpu.stat`：用于统计信息，包含`nr_periods`（表示经历了几个`cfs_period_us`周期）、`nr_throttled`（表示任务被限制的次数）及`throttled_time`（表示任务被限制的总时长）
    ...
    <img src="http://xwjpics.gumptlu.work/qinniu_uPic/image-20220831101500029.png" alt="image-20220831101500029" style="zoom:37%;" />

    > https://blog.51cto.com/u_13645741/3780803
...
> <font color='#e54d42'>**为什么CPU限制要设置不同的策略？（按限额、按比例分配？）**</font>
>
> * CFS绝对使用限制的限制是一个上限，并不是一定要用到此上限
> * 如果有其他模式的限制，例如shares比例份额，那么就会进一步的限制在其内
...
* 限额类
  ...
  * `memory.soft_limit_in_bytes`：表示软限制，只有<font color='#e54d42'>**比强制限制设置的值小**</font>时才有意义。填写格式同上。<font color='#e54d42'>当整体内存紧张的情况下，任务获取的内存就被限制在软限制额度之内</font>，以保证**不会有太多任务因内存挨饿**。可以看到，加入了内存的资源限制并不代表没有资源竞争
...
* 结构不同
  ...
  * 不同于 v1 版本
    ...
    * v1一般一个子系统单独一个层级树, 
    * cgroup v2 版本只有一个层级 树`Hierarchy`(层级).cgroup2 文件系统有一个根 Cgroup ，以 `0x63677270`数字来标识，**所有支持v2版本的子系统控制器会自动绑定到 v2的唯一层级上并绑定到根 Cgroup.**
...
<img src="http://xwjpics.gumptlu.work/qinniu_uPic/image-20220808080604973.png" alt="image-20220808080604973" style="zoom:50%;" />
...
对应到项目中docker就是会在容器的读写层复制一个修改的文件，因为容器的读取是从容器层往下读取，所以可以看到修改过的文件，但是底层的镜像文件是没有被修改的。在容器内删除一个文件的时候，会在读写层创建一个文件`.wh.file`(`whiteout`文件)来屏蔽底层文件的读，这样就相当于删除）
...
* 专门用于存放`/etc/hosts`、`/etc/resolv.conf`等信息，这些信息文件本来属于底层镜像只读层的一部分，但是用户往往在构建容器目录的时候需要改动这些特定的值
* 这些改动只对当前容器有效，不会影响底层镜像，同时不放在最上层的读写层的原因在于，当commit打包当前增量的/已修改过的容器的时候，**不期望将这些特有的操作系统文件信息一起打包**（init层不会参与打包），所以用一个init层来解决更为合适
...
是`DeviceMapper`中目标设备**动态分配**策略，每次按照需求分配需要的磁盘空间，避免资源的浪费（因为粒度小）
...
* chroot只是改变了当前进程的`/`
* switch_root和chroot类似，但是专门用于初始化系统的时候使用，不仅chroot还会删除旧根目录下的所有内容，释放内存。其只能由pid=1的进程使用
...
* `CMD`: 容器启动时运行的命令
  ...
  > <font color='#39b54a'>RUN是构件容器时就运行的命令以及提交运行结果，CMD是容器启动时执行的命令，在构建时并不运行，构建时仅仅指定了这个命令到底是个什么样子</font>
...
* 镜像系统：镜像系统可以使多个容器共享一个底层镜像，实现高效率的存储 （最为关键）
* 版本管理：类似于git的版本管理机制
...
## 33. 为什么commit打包镜像的时候挂载volume卷为空文件夹？（仅官方docker）

`mount --bind`的本质：将目录项指向的inode指针指向同一个inode

例如：`mount --bind /home /test	` ：将`home`文件夹挂载到`test`文件夹，linux内核的操作就是将`/test`目录项指向的`inode`指针指向`/home`目录项的`inode`，即两者共享一个底层`inode`

而docker的volume就是采用这样的方式实现（`mount bind`）, 所以容器内的`/test`目录的修改能够同步在宿主机`/home`，因为就是同一个`inode`

1. `/test`目录挂载在容器的读写层，为什么没有被`commit`呢？

   因为`docker commit`发生在宿主机视角，我们在容器中挂载了`volume`目录`/test`，对于宿主机来说是无法感知的，因为此时已经设置了`Mount NS`的挂载隔离，所以最后打包的时候并不知道挂载操作的存在（只知道`home`，挂载都是容器进程单方面的行为），所以该目录始终为空

2. 为什么有`/test`目录？

   因为挂载操作还是需要先创建目录的，`test`目录还是要先创建出来，而创建目录的操作，`Mount NS`是不会隔离的

3. 为什么读写层其他的目录都可以`commmit`?

   因为实现容器的读写层目录的挂载（`mount aufs`）发生在容器进程启动之前，此时还未`Mount NS`隔离，所以宿主机能够感知到

注：自己实现的docker项目中，打包时volume文件夹中有数据，原因在于我们挂载Volume的时机与挂载`mnt`目录同时，此时还是实现`Mount NS`的隔离
...
* Mount NS只是将挂载点隔离开了，但是子进程仍然是继承父进程的各个挂载点的，子进程如果没有新的挂载，那么就不会享受到Mount NS的隔离（所以也说明了在容器中的任何挂载也不会影响宿主机了），所以要重新挂载根目录
* 同时还需要主要原本宿主机根目录挂载的传播机制，如果是共享模式，此时挂载时还需要变为私有模式，这样才不会影响
...
<img src="http://xwjpics.gumptlu.work/qinniu_uPic/image-20220324224831428.png" alt="image-20220324224831428" style="zoom: 33%;" />

包的流转会经过veth的一端流向另一端到达宿主机上的docker0网桥，宿主机的docker0网桥需要对其开启iptables的forward转发规则，这样docker0网桥就可以将数据包流转到另一个容器的veth对上，最终到达另一个容器内部
