  ...
  - [ ] profile
    ...
    - [ ] 优化agent
      - [ ] 不同采集器接口化
        - [x] ebpf
        - [x] aysnc-profile ***
          - [x] ~~直接runcmd start阻塞的主要原因在于：会创建fdtransfer和jattach两个子进程, 后者立马结束，但是fdtransfer是一个常驻进程，所以一直存在，导致父进程一直阻塞，解决办法是分解单独执行（将具体的命令拆解出来）~~ => (用老版本的方式解决，不单独执行)
      - [ ] 程序编程语言类型判断优化
    ...
    - [ ] 适配Python ***
      ...
      - [x] rust lib静态包编译
    - [ ] 确定总体的一个采样周期，采集间隔
      - [ ] 初步拟定的方案：pyspy计算间隔时间，然后作为周期性snapshot的间隔时间
    - [x] 演讲（7月5日）
